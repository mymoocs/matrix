...
x = -9
y = 1/2
expr_for_check = 2**(y+1/2) if x+10<0 else 2**(y-1/2)
expression_val = 1.0
...
first_five_pows_two = { pow(2,n) for n in {0,1,2,3,4} }
...
cartesian_product = [[s,n] for s in ['A','B','C'] for n in [1,2,3]]
...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ... ] with a one-line list comprehension in which S appears
exclude_zero_list =[(i,j,k) for i in S for j in S for k in S if i != 0 and j != 0 and k != 0 and i+j+k == 0] 
...
first_of_tuples_list = [(i,j,k) for i in S for j in S for k in S if i != 0 and j != 0 and k != 0 and i+j+k == 0][0] 
...
list_sum_zip = [a+b for (a,b) in list(zip(A, B)) ]
...
square_dict = {k:k*k for k in list(range(100)) }
